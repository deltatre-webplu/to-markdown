...
function isBlock(node) {
  return ['address', 'article', 'aside', 'audio', 'blockquote', 'body', 'canvas', 'center', 'dd', 'dir', 'div', 'dl', 'dt', 'fieldset', 'figcaption', 'figure', 'footer', 'form', 'frameset', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'header', 'hgroup', 'hr', 'html', 'isindex', 'li', 'main', 'menu', 'nav', 'noframes', 'noscript', 'ol', 'output', 'p', 'pre', 'section', 'table', 'tbody', 'td', 'tfoot', 'th', 'thead', 'tr', 'ul'].indexOf(node.nodeName.toLowerCase()) !== -1;
}
...
function OptionsValidator() {
  var _this = this;
  this.validOptions = {};
  var validOptions = [['headingStyle', ['setext', 'atx']], ['hr', /([*-_] *){3,}/, 'needs to be a sequence of three of more characters matching -, _, or *, each followed optionally by any number of spaces, e.g. * * *'], ['bulletListMarker', ['*', '-', '+']], ['codeBlockStyle', ['indented', 'fenced']], ['fence', ['```', '~~~']], ['emDelimiter', ['_', '*']], ['strongDelimiter', ['__', '**']], ['linkStyle', ['inlined', 'referenced']], ['linkReferenceStyle', ['full', 'collapsed', 'shortcut']], ['br', ['  ', '\\']]];

  validOptions.forEach(function (validOption) {
    _this.validOptions[validOption[0]] = ValidOption.apply(null, validOption);
  });
}
...
function ValidOption(name, values, customMessage) {
  ...
  this.name = name;
  this.values = values;
  this.message = this.name + ' ' + (customMessage || 'needs to be either: ' + toSentence(this.values));
}
...
function toSentence(array) {
  ...
  for (var i = 1; i < length; i++) {
    var item = array[i];
    if (i === length - 1) sentence = sentence + lastWordConnector + item;else sentence = sentence + ', ' + item;
  }
  return sentence;
}
...
function isVoid$1(node) {
  return ['area', 'base', 'br', 'col', 'command', 'embed', 'hr', 'img', 'input', 'keygen', 'link', 'meta', 'param', 'source', 'track', 'wbr'].indexOf(node.nodeName.toLowerCase()) !== -1;
}

function isBlank(node) {
  return !isVoid$1(node) && !/A|TH|TD/.test(node.nodeName) && /^\s*$/i.test(node.textContent);
}
